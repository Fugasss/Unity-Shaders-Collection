Shader "Custom/UI/SizePulseVertexBased"
{
    Properties
    {
        ...
        _Pivot("UI Element Pivot", Vector) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline float easeInOutSine(in float x)
            {
                return -(cos(UNITY_PI * x) - 1) / 2;
            }

            inline float4 scaleVertex(in float4 vertex)
            {
                vertex -= _Pivot;
                vertex *= lerp(_MinScale, _MaxScale, easeInOutSine(_Speed * _Time.y));
                vertex += _Pivot;
            ...
            v2f VectorVert(appdata_t IN)
            {
                ...
                OUT.vertex = scaleVertex(OUT.vertex);
